...
async def run_remote_shutdown(device: Device) -> tuple[int | None, str]:
    arguments = build_shutdown_command(device)
    try:
    ...
    try:
        stdout, stderr = await asyncio.wait_for(
            process.communicate(), timeout=device.shutdown.command_timeout_seconds
        )
    except TimeoutError:
        process.kill()
...
async def shutdown_device(device: Device, settings: Settings) -> dict[str, Any]:
    ...
    command_exit_code, command_error = await run_remote_shutdown(device)
